...
boot_alloc(uint32_t n)
{
    ...
    if (PGSIZE * npages < PADDR(nextfree)){
        panic("boot_alloc panic");
    }
    return result;
#ifdef SANITIZE_SHADOW_BASE
...
page_init(void)
{
	...
	//     page tables and otALLOC_ZERher data structures?
	//
...
{
	pte_t* ptep;
	struct PageInfo * pi = page_lookup(pgdir, va, &ptep);
	if (pi){
		page_decref(pi);
        *ptep = 0;
